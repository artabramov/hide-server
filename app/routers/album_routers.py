...
from app.schemas.album_schemas import AlbumSchema, AlbumInsertValidator, AlbumUpdateValidator, AlbumsListValidator
...
async def insert_album(session=Depends(get_session), cache=Depends(get_cache), schema=Depends(AlbumInsertValidator),
...
async def update_album(album_id: int, session=Depends(get_session), cache=Depends(get_cache),
                       schema=Depends(AlbumUpdateValidator), current_user=Depends(auth_editor)):
    """Update album."""
    try:
        album_repository = AlbumRepository(session, cache)
        album = await album_repository.select(album_id)
...
async def albums_list(session = Depends(get_session), cache = Depends(get_cache),
                     schema = Depends(AlbumsListValidator), current_user=Depends(auth_reader)):
    """Get users list."""
    album_repository = AlbumRepository(session, cache)
