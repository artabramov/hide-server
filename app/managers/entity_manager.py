...
class EntityManager:
    ...
    async def select(self, cls: object, obj_id: int) -> object:
        ...
        log.debug("Select SQLAlchemy object from Postgres database by id, cls=%s, obj_id=%s, obj=%s" % (
    ...
    async def exists(self, cls: object, **kwargs) -> bool:
        ...
        log.debug("Check if object exists in Postgres database, cls=%s, kwargs=%s, result=%s." % (
            str(cls.__name__), str(kwargs), result))
        return result
